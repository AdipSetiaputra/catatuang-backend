...
class DashboardController extends Controller
{
    ...
    public function summary(Request $request): JsonResponse
    {
        ...
        $totalMasuk = $todayTransactions->where('type', 'masuk')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        $totalKeluar = $todayTransactions->where('type', 'keluar')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        ...
        $monthlyMasuk = $monthlyTransactions->where('type', 'masuk')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        $monthlyKeluar = $monthlyTransactions->where('type', 'keluar')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        ...
        $weeklyMasuk = $weeklyTransactions->where('type', 'masuk')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        $weeklyKeluar = $weeklyTransactions->where('type', 'keluar')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount');
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $chartData[] = [
                'date' => $displayDate,
                'masuk' => $dayTxs->where('type', 'masuk')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount'),
                'keluar' => $dayTxs->where('type', 'keluar')->whereNotIn('source', ['SISTEM_TRANSFER', 'MODAL', 'MUTASI'])->sum('amount'),
            ];
        }
